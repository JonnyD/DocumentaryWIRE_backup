...
namespace DW\DocumentaryBundle\Repository\Doctrine\ORM;
 ...
 */
class DocumentaryRepository extends EntityRepository implements DocumentaryRepositoryInterface
{
    public function findDocumentaryById($id)
    {
        $query = $this->getEntityManager()
            ->createQuery('
               SELECT d, c FROM DocumentaryBundle:Documentary d
               JOIN d.category c
               WHERE d.id = :id')
            ->setParameter('id', $id);
    ...
    public function findFeaturedDocumentaries()
    {
        $query = $this->getEntityManager()->createQuery('
               SELECT d FROM DocumentaryBundle:Documentary d
                WHERE d.featured = 1
    ...
    public function findDocumentariesByTag(Tag $tag)
    {
        $query = $this->getEntityManager()
            ->createQuery('SELECT d
                FROM DocumentaryBundle:Documentary d
                JOIN d.tags t
                WHERE t.slug = :slug')
